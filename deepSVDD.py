...
class DeepSVDD(object):
    ...
    def train(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 50,
              lr_milestones: tuple = (), batch_size: int = 128, weight_decay: float = 1e-6,J: int = 2, device: str = 'cuda',
        ...
        self.trainer = DeepSVDDTrainer(self.objective, self.R, self.c, self.nu, optimizer_name, lr=lr,
                                       ...
                                       weight_decay=weight_decay, J=J, device=device, n_jobs_dataloader=n_jobs_dataloader)
        # Get the model
    ...
    def test(self, dataset: BaseADDataset,J: int = 2, device: str = 'cuda', n_jobs_dataloader: int = 0,path='D/Omid'):
        """Tests the Deep SVDD model on the test data."""

        if self.trainer is None:
            self.trainer = DeepSVDDTrainer(self.objective, self.R, self.c, self.nu, J=J,
                                           device=device, n_jobs_dataloader=n_jobs_dataloader)

        idx,out=self.trainer.test(dataset, self.net)
        np.save(path+"/Embeding.npy", out)
        # Get results
        self.results['test_auc'] = self.trainer.test_auc
        self.results['test_time'] = self.trainer.test_time
        self.results['test_scores'] = self.trainer.test_scores
        out_file = open(path+ '/sorted.txt','w')
    ...
    def pretrain(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 100,
        ...
        self.ae_trainer = AETrainer(optimizer_name, lr=lr, n_epochs=n_epochs, lr_milestones=lr_milestones,
                                    batch_size=batch_size, weight_decay=weight_decay,J=J, device=device,
    ...
    def load_model(self, model_path, load_ae=True):
    ...
    def load_ae_model(self, model_path, load_ae=True):
        """Load Deep SVDD model from model_path."""
